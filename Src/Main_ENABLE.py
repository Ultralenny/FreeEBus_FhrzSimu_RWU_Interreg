...
if __name__ == "__main__":
    param = build_Mercedes_eCitaro()

    ###_________________Lookuptable_Wirkungsgrad___________________________________________#####
    ...
    default_route = r"Data\Fahrprofil_Flo\route_profile_fhv_flo.csv"

    path_RouteProfile = os.getenv("FREEEBUS_SPEED_PROFILE", default_route)
    ...
    ###____________________Route_Profile______________________________####
    print("_Functioncall_Route_Profile_")
    route = load_route_profile(path_RouteProfile)
    velocity_array_ms = route['velocity_ms'].to_numpy(float)
    strecke_array_m = route['s_m'].to_numpy(float)
    Steigung_array_rad = route['slope_rad'].to_numpy(float)
    ...
    for index in range(len(velocity_array_ms) - 1):
        ...
        if velocity < velocity_threshold:  # Bus steht
            stillstand_counter += dt
        else:  # Bus fährt
            stillstand_counter = 0
    ...
    v = velocity_array_ms * 3.6
    t = np.arange(len(velocity_array_ms)) * dt
